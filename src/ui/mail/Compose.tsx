...
export function Compose({account, onClose, initialTo, initialSubject, initialBody, initialAttachments, signatureConfig, onSetSignatureEnabled, onSetSignatureContent}: {account: Account; onClose: () => void; initialTo?: string; initialSubject?: string; initialBody?: string; initialAttachments?: Attachment[]; signatureConfig?: SignatureConfig; onSetSignatureEnabled?: (enabled: boolean) => void; onSetSignatureContent?: (content: string, format: 'text'|'html') => void}) {
  ...
  function moveLeft() { setCursor((c) => Math.max(0, c - 1)) }
  function moveRight() { setCursor((c) => Math.min(body.length, c + 1)) }
  ...
  useInput((input, key) => {
    ...
    if (key.tab) {
      ...
      if (mode === 'insert') {
        ...
        if (key.backspace || ch === '\\x7f' || ch === '\\b') { backspace(); return }
        if (ch === 'i') setMode('insert')
        else if (ch === 'a') { moveRight(); setMode('insert') }
        else if (ch === 'I') { moveStartOfLine(); setMode('insert') }
        else if (ch === 'A') { moveEndOfLine(); setMode('insert') }
        else if (ch === 'o') { moveEndOfLine(); newlineAtCursor(); setMode('insert') }
        else if (ch === 'O') { moveStartOfLine(); insertText('\n'); setCursor((c) => Math.max(0, c - 1)); setMode('insert') }
  ...
  async function doSend() {
    ...
    try {
      ...
      const html = buildHtmlEmail({subject, bodyText: body, signatureText: sigEnabled && !sigHtml ? signatureConfig?.content : undefined, signatureHtml: sigEnabled ? sigHtml : undefined})
  ...
  return (
    <Box flexDirection="column">
      ...
      <Box>
        <Text dimColor>
          Words: {useMemo(() => (body.trim().length ? body.trim().split(/\s+/).length : 0), [body])} · Letters: {body.length}
        </Text>
      </Box>
      {error && (
        <Box marginTop={1}>
      ...
      {sending ? (
        ...
        <>
          <Box marginTop={1}>
            <Text dimColor>
              {focus === 'footer' ? '› ' : ''}See instructions (Enter)
            </Text>
          </Box>
          {showInstructions && (
            <Box marginTop={1}>
              <Text dimColor>
                Body uses Vim editing — normal/insert modes. In normal: h j k l move · w/b next/prev word · i/a/I/A/o/O insert · x delete char · dd delete line · dw/db delete to next/prev word · yy yank line · p paste · 0/$ line start/end. Backspace deletes in insert or normal. Esc leaves insert. Tab/Shift+Tab navigate · ctrl+enter newline · ctrl+s send · esc cancel · ctrl+a add file · ctrl+r remove last · ctrl+d clear attachments · ctrl+g toggle signature · ctrl+G set signature file
              </Text>
            </Box>
          )}
        </>
